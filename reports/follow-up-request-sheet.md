...
| Better file unit identified | New file-unit request | Response names a better folder, OA/ID, lot file, box, series, or NAID locator. | Parent request ID, new repository control identifiers, any public locator identifiers, relationship to original target, and exact scan/MDR/folder pull ask. | Add child request row in [ledger](request-tracking-ledger.html) and update [send sheet](request-send-sheet.html) if it becomes priority. |
