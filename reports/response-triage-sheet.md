...
| Fully released control copy | Released | Record file name, page count, date received, and release status in the ledger. | Draft or complete the relevant [source-note worklist](source-note-worklist.html) row: repository, collection, series, folder, source-control identifier such as OA/ID, lot, box/folder, document/cable number, classification, document type, attachments, and declassification markings. Keep NAID/public URL as locator or tracking data unless no better identifier exists. | Run the [promotion gate checklist](promotion-gate-checklist.html); if all gates clear, update the [selection worksheet](selection-worksheet.html). |
