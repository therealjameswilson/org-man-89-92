...
| Control identifiers | Repository, collection, series, box/folder, OA/ID, local ID, lot number, document/cable number, file unit, and any NAID/public URL as locator data only. | Supplies the backbone of a FRUS source note while separating archival provenance from public discovery links. | [Source note worklist](source-note-worklist.html) |
...
| Better file unit identified | New file unit, series/folder, OA/ID or other repository control identifier, any NAID/public locator, and why it is better than the original target. | Do not abandon the parent request without preserving the chain. |
